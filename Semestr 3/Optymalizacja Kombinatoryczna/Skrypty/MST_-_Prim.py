...
#         key = [2^63-1]* self.V            #[sys.maxint] * self.V
...
def AlgorytmPrima(macierz, s):
	...
	while (len(kolejka) != 0):
		...
		kolejka.remove(u)
		print("Ten wierzcholek juz mamy w MST", str(u))
	return wynik

n = int(input("Ile wierzchoĹkĂłw ma graf?\n n = "))
s = int(input("Od ktĂłrego wierzchoĹka zaczÄÄ?\n s = "))
print("Podaj macierz:")
macierz = [[int(x) for x in input().split()] for y in range(n)]
wynik = AlgorytmPrima(macierz, s)
suma = 0
print("ID\tPoprzednik   - krawedzie zawarte w mst (cokolwiek i -1 to punkt startowy")
for i in range(n):
	print(str(i) + "\t" + str(wynik[i].poprzednik))
	suma += wynik[i].koszt
print("laczny koszt wynosi: " + str(suma))