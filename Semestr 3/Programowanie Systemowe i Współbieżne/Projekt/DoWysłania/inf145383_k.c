...
int main(int argc, char *argv[])
{
	...
	*async = 0;
	...
	if(connect(QID)) // Sprawdzenie czy połączono z serwerem (niestety prowizoryczne)
	{
		while(1) // Główna pętla
		{
			...
			else if(strcmp(action, "async")==0) // Tryb asynchroniczny
			{
				*async ^= 1;
				...
				{
					printf("Dezaktywowano odbior asynchroniczny 2.\n");
					kill(asyncpid, 9);
				}
			}
			...
			else if(strcmp(action, "bans")==0) // [Debug] Lista blokad
			{
				bans(QID);
			}
